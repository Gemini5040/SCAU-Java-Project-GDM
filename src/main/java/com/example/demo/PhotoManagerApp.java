...
public class PhotoManagerApp extends Application {
    ...
    private Pane thumbnailContainer;
    ...
    private VBox createSidebarWithHeader() {
        ...
        sidebarTitle.setStyle(
            "-fx-font-size: 13px; -fx-font-weight: 700; -fx-text-fill: " +
                TEXT_SIDEBAR +
                "; -fx-font-family: 'Segoe UI', sans-serif; -fx-letter-spacing: 1;"
        );
    ...
    private HBox createAddressBar() {
        ...
        addressField.setOnAction(e ->
            navigateToPath(addressField.getText().trim())
        );
        folderBtn.setOnAction(e -> chooseFolder());

        goBtn.setOnMouseEntered(e ->
            goBtn.setStyle(goBtn.getStyle().replace(PRIMARY, "#4F46E5"))
        );
        goBtn.setOnMouseExited(e ->
            goBtn.setStyle(
                "-fx-background-color: " +
                    PRIMARY +
                    ";" +
    ...
    private void chooseFolder() {
        ...
        File selected = chooser.showDialog(
            directoryTree.getScene().getWindow()
        );
        if (selected != null) {
    ...
    private void expandTreeTo(File targetDir) {
        ...
        if (driveItem == null) {
            statusLabel.setText(
                "Navigating to: " + targetDir.getAbsolutePath()
            );
    ...
    private TreeItem<File> findDriveRoot(
        TreeItem<File> root,
        String targetPath
    ) {
    ...
    private String normalizePath(String path) {
        ...
        if (!p.endsWith("\\")) p = p + "\\";
        if (p.length() >= 2 && p.charAt(1) == ':' && p.length() == 2) p =
            p + "\\";
        return p;
    }

    private TreeItem<File> walkDownPath(
        TreeItem<File> start,
        String targetPath
    ) {
        File startVal = start.getValue();
        String startPath =
            startVal != null ? normalizePath(startVal.getAbsolutePath()) : "";
        ...
        for (String segment : segments) {
            ...
            for (TreeItem<File> child : children) {
                ...
                if (
                    childVal != null &&
                    childVal.getName().equalsIgnoreCase(segment)
                ) {
                    found = child;
                    break;
                }
    ...
    private HBox createToolbar() {
        ...
        refreshBtn.setOnAction(e -> {
            if (currentDirectory != null) loadImagesFromDirectory(
                currentDirectory
            );
        });
    ...
    private void applyTreeViewStyle(TreeView<?> tree) {
        tree.setStyle(
            "-fx-background-color: transparent; -fx-selection-bar: " +
                PRIMARY +
    ...
    private void initThumbnailArea() {
        ...
        // Bug2修复：用 Pane 替换 StackPane，Pane 不做自动居中布局
        // Rectangle 的 setX/setY 才能准确对应鼠标坐标
        thumbnailContainer = new Pane();
        thumbnailPane
            .prefWidthProperty()
            .bind(thumbnailContainer.widthProperty());
        ...
        // Pane 不自动布局子节点，fitToHeight(true) 会裁切 TilePane 导致无滚动
        // 设为 false 让 Pane 高度由 TilePane 内容驱动，超出视口时自动出滚动条
    ...
    private void applyToolbarTitleStyle(Label label) {
        label.setStyle(
            "-fx-font-size: 14px; -fx-font-weight: 600; -fx-text-fill: " +
                TEXT_PRIMARY +
    ...
    private void applyCountBadgeStyle(Label label) {
        label.setStyle(
            "-fx-background-color: " +
                PRIMARY +
                "; -fx-text-fill: white; -fx-font-size: 11px; -fx-font-weight: bold; -fx-padding: 4 10; -fx-background-radius: 10; -fx-cursor: default;"
        );
    ...
    private void applyStatusBadgeStyle(Label label) {
        label.setStyle(
            "-fx-background-color: " +
                PRIMARY +
                "; -fx-text-fill: white; -fx-font-size: 11px; -fx-font-weight: bold; -fx-padding: 4 12; -fx-background-radius: 10; -fx-cursor: default;"
        );
        label.setVisible(false);
    }
    ...
    private Button createModernButton(
        ...
        btn.setStyle(
            "-fx-background-color: " +
                color +
                "; -fx-text-fill: " +
                (color.equals(BG_MAIN) ? TEXT_PRIMARY : "white") +
                ";" +
        ...
        btn.setOnMouseEntered(e ->
            btn.setStyle(
                btn
                    .getStyle()
                    .replace(color, darkenColor(color))
                    .replace("0.08", "0.15")
            )
        );
    ...
    // Bug3修复：三种状态的 border-width 统一为 2.5，仅改颜色
    // 宽度不变则 TilePane 不会重新计算所有卡片尺寸，消除闪动
    private void applyNormalStyle(VBox node) {
        node.setStyle(
            "-fx-background-color: " +
                BG_CARD +
                "; -fx-background-radius: 10;" +
                "-fx-border-color: transparent; -fx-border-width: 2.5; -fx-border-radius: 10;" +
                "-fx-padding: 8;" +
    ...
    private void handleMousePressForSelection(MouseEvent e) {
        ...
        // 精确检测：仅当点击容器本身或 TilePane 空白区域时才启动框选
        // 不能用 instanceof Pane，因为 VBox 也是 Pane 的子类，会误触发
        if (target == thumbnailContainer || target == thumbnailPane) {
            isSelecting = true;
            startX = e.getX();
            startY = e.getY();
    ...
    private void handleMouseDragForSelection(MouseEvent e) {
        if (isSelecting) {
            ...
            selectionRectangle.setWidth(Math.abs(e.getX() - startX));
            selectionRectangle.setHeight(Math.abs(e.getY() - startY));
        }
    }
    ...
    private void handleMouseReleaseForSelection(MouseEvent e) {
        if (isSelecting) {
            ...
            selectInRect(
                selectionRectangle.getBoundsInParent(),
                e.isControlDown()
            );
        }
    }
    ...
    private void selectInRect(Bounds rect, boolean add) {
        ...
        for (var node : thumbnailPane.getChildren()) {
            if (node instanceof VBox) {
                ...
                Bounds rInScene = thumbnailPane.localToScene(
                    new BoundingBox(
                        rect.getMinX(),
                        rect.getMinY(),
                        rect.getWidth(),
                ...
                if (
                    rInScene.intersects(bInScene) &&
                    !selectedNodes.contains(box)
                ) {
                    selectedNodes.add(box);
    ...
    private void handleSelection(VBox node, boolean ctrl) {
        ...
        if (selectedNodes.contains(node) && ctrl) {
            selectedNodes.remove(node);
            applyNormalStyle(node);
        } else {
    ...
    private List<TreeItem<File>> buildChildren(TreeItem<File> p) {
        ...
        if (d != null && d.isDirectory()) {
            File[] fs = d.listFiles(f -> f.isDirectory() && !f.isHidden());
            if (fs != null) {
                Arrays.sort(
                    fs,
    ...
    private void loadImagesFromDirectory(File dir) {
        ...
        clearSelection();
        currentImageIndex = -1;
        File[] files = dir.listFiles(this::isImageFile);
        if (files == null) {
            statusLabel.setText("Cannot access this directory");
        ...
        for (File file : files) {
            try {
                ...
                total += file.length();
                thumbnailPane.getChildren().add(createThumbnailNode(file));
                cnt++;
            } catch (Exception ex) {
        ...
        directoryInfoLabel.setText(dir.getName());
        imageCountBadge.setText(cnt + " photos");
        imageCountBadge.setVisible(true);
        statusLabel.setText(
            String.format(
                "%d images loaded | Total: %s",
                cnt,
    ...
    private Image createHighQualityImage(File f, double w, double h) {
    ...
    private VBox createThumbnailNode(File file) {
        ...
        applyNormalStyle(box);
        Image img = createHighQualityImage(
            file,
            THUMBNAIL_SIZE,
            THUMBNAIL_SIZE
        );
        ...
        nameLbl.setStyle(
            "-fx-alignment:center;-fx-text-overrun:ellipsis;-fx-font-size:11px;-fx-font-weight:500;-fx-text-fill:" +
                TEXT_PRIMARY +
                ";"
        );
        Label sizeLbl = new Label(formatFileSize(file.length()));
        sizeLbl.setStyle(
            "-fx-alignment:center;-fx-font-size:10px;-fx-text-fill:" +
                TEXT_SECONDARY +
                ";"
        );
        box.getChildren().addAll(iv, nameLbl, sizeLbl);
        setupThumbnailEvents(box, file);
        return box;
    }
    ...
    private void setupThumbnailEvents(VBox box, File file) {
        box.setOnMouseClicked(e -> {
            if (e.getButton() == MouseButton.PRIMARY) {
                if (e.getClickCount() == 2) openSlideshow(
                    currentImages.indexOf(file)
                );
                else {
                    handleSelection(box, e.isControlDown());
                    currentImageIndex = currentImages.indexOf(file);
                }
            }
        });
        box.setOnMouseEntered(e -> {
            if (!selectedNodes.contains(box)) applyHoverStyle(box);
        });
        box.setOnMouseExited(e -> {
            if (!selectedNodes.contains(box)) applyNormalStyle(box);
        });
    }
    ...
    private void navigateImage(int direction) {
        ...
        int newIndex = Math.max(
            0,
            Math.min(currentImages.size() - 1, currentImageIndex + direction)
        );
    ...
    private void updateStatusAfterSelection() {
        if (selectedNodes.isEmpty() && currentDirectory != null) {
            statusLabel.setText(
                currentImages.size() + " images in folder | No selection"
            );
            selectedCountLabel.setVisible(false);
        } else if (!selectedNodes.isEmpty()) {
            long sz = selectedNodes
                .stream()
                .mapToLong(n -> ((File) n.getUserData()).length())
                .sum();
            statusLabel.setText(
                String.format(
                    "%d selected | %s",
            ...
            selectedCountLabel.setText(selectedNodes.size() + " Selected");
    ...
    private ContextMenu createContextMenu() {
        ...
        MenuItem c = new MenuItem("Copy"),
            p = new MenuItem("Paste"),
            r = new MenuItem("Rename"),
            d = new MenuItem("Delete");
        c.setOnAction(e -> copySelectedFiles());
        p.setOnAction(e -> pasteFiles());
        r.setOnAction(e -> renameFiles());
        d.setOnAction(e -> deleteFiles());
        m
            .getItems()
            .addAll(
                c,
    ...
    private void copySelectedFiles() {
        ...
        clipboardFiles = selectedNodes
            .stream()
            .map(n -> (File) n.getUserData())
            .collect(Collectors.toList());
        statusLabel.setText("Copied " + clipboardFiles.size() + " images");
    }
    ...
    private void deleteFiles() {
        ...
        Alert a = new Alert(
            ...
            "Delete " + selectedNodes.size() + " images?",
            ButtonType.YES,
            ButtonType.NO
        );
        ...
        if (r.isPresent() && r.get() == ButtonType.YES) {
            int ok = 0;
            for (VBox n : selectedNodes)
                if (((File) n.getUserData()).delete()) ok++;
            statusLabel.setText("Deleted: " + ok + "/" + selectedNodes.size());
    ...
    private void pasteFiles() {
        ...
        statusLabel.setText("Pasted: " + ok + " images");
        loadImagesFromDirectory(currentDirectory);
    }

    private File resolveDest(File src) {
        File d = new File(currentDirectory, src.getName());
        int c = 1;
        String base = getBaseName(src.getName()),
            ext = getExtension(src.getName());
        while (d.exists()) d = new File(
            currentDirectory,
            base + "_copy(" + (c++) + ")" + ext
        );
        return d;
    ...
    private void renameFiles() {
        ...
        List<File> list = selectedNodes
            .stream()
            .map(n -> (File) n.getUserData())
            .collect(Collectors.toList());
        if (list.size() == 1) showSingleRename(list.get(0));
        else showBatchRename(list);
    }

    private void showSingleRename(File f) {
        String cur = getBaseName(f.getName());
        ...
        d
            .showAndWait()
            .ifPresent(nm -> {
                if (nm.trim().isEmpty() || nm.equals(cur)) {
                    showAlert("Info", "Invalid!");
                    return;
                }
                File nf = new File(
                    f.getParent(),
                    nm.trim() + getExtension(f.getName())
                );
                if (nf.exists()) {
                ...
                if (f.renameTo(nf)) {
                    statusLabel.setText("Renamed: " + nm);
                    loadImagesFromDirectory(currentDirectory);
    ...
    private void showBatchRename(List<File> files) {
        ...
        dlg.setTitle("Batch Rename");
        dlg.setHeaderText("Rename " + files.size() + " files");
        GridPane g = new GridPane(15, 12);
        g.setPadding(new Insets(20));
        TextField pre = new TextField("NewName"),
            st = new TextField("1"),
            pd = new TextField("4");
        g.addRow(0, new Label("Prefix:"), pre);
        g.addRow(1, new Label("Start#:"), st);
        g.addRow(2, new Label("Digits:"), pd);
        Label pv = new Label("Preview: NewName0001.jpg...");
        pv.setStyle(
            "-fx-text-fill:" + TEXT_SECONDARY + ";-fx-font-style:italic;"
        );
        g.addRow(3, pv, pv);
        pre
            .textProperty()
            .addListener((o, a, b) ->
                updatePreview(pv, b, st.getText(), pd.getText())
            );
        st
            .textProperty()
            .addListener((o, a, b) ->
                updatePreview(pv, pre.getText(), b, pd.getText())
            );
        pd
            .textProperty()
            .addListener((o, a, b) ->
                updatePreview(pv, pre.getText(), st.getText(), b)
            );
        ...
        dlg.setResultConverter(b ->
            b == ButtonType.OK
                ? new String[] { pre.getText(), st.getText(), pd.getText() }
                : null
        );
        dlg.showAndWait().ifPresent(r -> execBatchRename(files, r));
    }

    private void execBatchRename(List<File> fs, String[] p) {
        try {
            String pr = p[0].isEmpty() ? "Image" : p[0];
            int s = Integer.parseInt(p[1].isEmpty() ? "1" : p[1]),
                n = Integer.parseInt(p[2].isEmpty() ? "4" : p[2]);
            String fmt = "%0" + Math.max(n, 1) + "d";
            int ok = 0;
            for (File f : fs) {
                String ext = getExtension(f.getName()),
                    nm = pr + String.format(fmt, s++) + ext;
                File nf = new File(f.getParent(), nm);
                if (!nf.exists() && f.renameTo(nf)) ok++;
            }
            statusLabel.setText("Renamed: " + ok + "/" + fs.size());
            loadImagesFromDirectory(currentDirectory);
    ...
    private void updatePreview(Label l, String pre, String s, String p) {
        try {
            int si = s.isEmpty() ? 1 : Integer.parseInt(s),
                pi = p.isEmpty() ? 4 : Integer.parseInt(p);
            String f = "%0" + Math.max(pi, 1) + "d",
                pr = pre.isEmpty() ? "NewName" : pre;
            l.setText(
                "Preview: " +
                    pr +
                    String.format(f, si) +
                    ".jpg, " +
                    pr +
                    String.format(f, si + 1) +
                    ".jpg..."
            );
        } catch (Exception ex) {
            l.setText("Preview: [enter valid numbers]");
        }
    }

    // ==================== Slideshow ====================

    private void openSlideshow(int idx) {
        if (currentImages.isEmpty()) {
            showAlert("Info", "No images!");
            return;
        }
        Stage stage = new Stage();
        stage.setTitle("Slideshow - " + currentDirectory.getName());
        stage.initOwner(directoryTree.getScene().getWindow());
        AtomicInteger ci = new AtomicInteger(
            Math.max(0, Math.min(idx, currentImages.size() - 1))
        );
        ...
        iv.setSmooth(true);
        iv.setCache(true);
        iv.setFitWidth(SLIDEVIEW_WIDTH);
        iv.setFitHeight(SLIDEVIEW_HEIGHT);
        iv.setStyle("-fx-background-color:#0F172A;");
        Label info = createSlideInfoLabel();
        Runnable upd = () -> {
            if (currentImages.isEmpty()) return;
            File f = currentImages.get(ci.get());
            Image img = createHighQualityImage(f, 1920, 1440);
            iv.setImage(img);
            zoom.set(1.0);
            iv.setScaleX(1);
            iv.setScaleY(1);
        ...
        upd.run();
        Timeline tl = createTimeline(ci, upd, interval);
        HBox bar = createSlideControls(iv, ci, zoom, interval, tl, upd, info);
        BorderPane sp = new BorderPane();
        StackPane center = new StackPane(iv, info);
        center.setStyle(
            "-fx-background-color:linear-gradient(135deg,#0F172A 0%,#1E293B 50%,#334155 100%);"
        );
        StackPane.setAlignment(info, Pos.BOTTOM_LEFT);
        sp.setCenter(center);
        sp.setBottom(bar);
        Scene sc = new Scene(sp, 1050, 780);
        stage.setScene(sc);
        setupSlideKeys(sc, ci, upd, tl);
        setupSlideScroll(iv, zoom, info);
        stage.setOnCloseRequest(e -> tl.stop());
        stage.show();
    }
    ...
    private Label createSlideInfoLabel() {
        Label l = new Label();
        l.setStyle(
            "-fx-text-fill:white;-fx-font-size:13px;-fx-font-family:'Consolas','Monaco',monospace;-fx-padding:8 14;-fx-background-color:rgba(0,0,0,0.75);-fx-background-radius:6;"
        );
        return l;
    }
    ...
    private Timeline createTimeline(
        ...
        return new Timeline(
            new KeyFrame(Duration.seconds(ir.get()), e -> {
                ci.set((ci.get() + 1) % currentImages.size());
                u.run();
            })
        );
    }

    private HBox createSlideControls(
        ImageView iv,
        AtomicInteger ci,
        ...
        Button prev = createSlideBtn("\u25C0", "Prev", "#60A5FA"),
            next = createSlideBtn("\u25B6", "Next", "#60A5FA"),
            zin = createSlideBtn("+", "Zoom", "#34D399"),
            zout = createSlideBtn("-", "Zoom", "#34D399"),
            fit = createSlideBtn("\u2630", "Fit", "#A78BFA"),
            play = createSlideBtn("\u25B6", "Play", "#FBBF24");
        Spinner<Double> sp = new Spinner<>(0.5, 10.0, ir.get(), 0.5);
        sp.setEditable(true);
        sp.setPrefWidth(70);
        sp
            .valueProperty()
            .addListener((o, a, nv) -> {
                ...
                if (tl.getStatus() == Animation.Status.RUNNING) {
                    tl.stop();
                    createTimeline(ci, u, ir).play();
                }
            });
        HBox ibox = new HBox(5, new Label("Interval:"), sp, new Label("s"));
        ibox.setAlignment(Pos.CENTER);
        ibox.setStyle(
            "-fx-padding:6 12;-fx-background-color:rgba(255,255,255,0.08);-fx-background-radius:6;"
        );
        prev.setOnAction(e -> navPrev(ci, u));
        next.setOnAction(e -> navNext(ci, u));
        zin.setOnAction(e -> zoomIn(zl, iv, info));
        zout.setOnAction(e -> zoomOut(zl, iv, info));
        fit.setOnAction(e -> {
            zl.set(1.0);
            iv.setScaleX(1);
            iv.setScaleY(1);
        ...
        play.setOnAction(e -> togglePlay(tl, play));
        HBox bar = new HBox(
            10,
            prev,
            next,
    ...
    private Button createSlideBtn(String text, String tip, String col) {
        Button b = new Button(text + " " + tip);
        b.setStyle(
            "-fx-background-color:" +
                col +
                ";-fx-text-fill:#0F172A;-fx-font-weight:bold;-fx-font-size:11px;-fx-padding:7 16;-fx-cursor:hand;-fx-background-radius:6;"
        );
        b.setOnMouseEntered(e ->
            b.setStyle(b.getStyle().replace(col, darkenColor(col)))
        );
    ...
    private void navPrev(AtomicInteger i, Runnable u) {
        if (i.get() == 0) showAlert("Info", "First!");
        else {
            i.decrementAndGet();
            u.run();
        }
    ...
    private void navNext(AtomicInteger i, Runnable u) {
        if (i.get() >= currentImages.size() - 1) showAlert("Info", "Last!");
        else {
            i.incrementAndGet();
            u.run();
        }
    ...
    private void zoomIn(AtomicReference<Double> z, ImageView iv, Label l) {
        double nz = Math.min(z.get() + ZOOM_STEP, MAX_ZOOM);
        z.set(nz);
        iv.setScaleX(nz);
        iv.setScaleY(nz);
    }

    private void zoomOut(AtomicReference<Double> z, ImageView iv, Label l) {
        double nz = Math.max(z.get() - ZOOM_STEP, MIN_ZOOM);
        z.set(nz);
        iv.setScaleX(nz);
        iv.setScaleY(nz);
    }

    private void togglePlay(Timeline tl, Button b) {
        if (tl.getStatus() == Animation.Status.RUNNING) {
            tl.stop();
            b.setText("\u25B6 Play");
            b.setStyle(
                "-fx-background-color:#FBBF24;-fx-text-fill:#0F172A;-fx-font-weight:bold;-fx-font-size:11px;-fx-padding:7 16;-fx-cursor:hand;-fx-background-radius:6;"
            );
        } else {
            tl.play();
            b.setText("\u23F8 Stop");
            b.setStyle(
                "-fx-background-color:" +
                    ACCENT +
                    ";-fx-text-fill:white;-fx-font-weight:bold;-fx-font-size:11px;-fx-padding:7 16;-fx-cursor:hand;-fx-background-radius:6;"
            );
        }
    }

    private void setupSlideScroll(
        ImageView iv,
        AtomicReference<Double> zl,
        Label il
    ) {
        iv.setOnScroll(e -> {
            if (e.isControlDown()) {
                if (e.getDeltaY() > 0) zoomIn(zl, iv, il);
                else if (e.getDeltaY() < 0) zoomOut(zl, iv, il);
                e.consume();
            }
        });
    }

    private void setupSlideKeys(
        Scene s,
        AtomicInteger ci,
        Runnable u,
        Timeline tl
        ...
        s.addEventHandler(KeyEvent.KEY_PRESSED, e -> {
            switch (e.getCode()) {
                ...
                case DOWN:
                case SPACE:
                    if (ci.get() < currentImages.size() - 1) {
                        ci.incrementAndGet();
                        u.run();
                    }
                    break;
    ...
    private String formatFileSize(long s) {
        ...
        if (s < 1024 * 1024 * 1024) return String.format(
            "%.1f MB",
            s / (1024.0 * 1024.0)
        );
        return String.format("%.1f GB", s / (1024.0 * 1024.0 * 1024.0));
    }
    ...
    private String darkenColor(String h) {
    ...
    private void setupSelectionRectangle(Pane c) {
        selectionRectangle = new Rectangle();
        selectionRectangle.setFill(Color.color(0.39, 0.37, 0.95, 0.12));
        selectionRectangle.setStroke(Color.color(0.39, 0.37, 0.95, 0.75));
        selectionRectangle.getStrokeDashArray().addAll(8.0, 4.0);
        selectionRectangle.setVisible(false);
        // 矩形本身不拦截鼠标事件，让拖拽事件穿透到容器
        selectionRectangle.setMouseTransparent(true);
        c.getChildren().add(selectionRectangle);
        c.setOnMousePressed(this::handleMousePressForSelection);
        c.setOnMouseDragged(this::handleMouseDragForSelection);
        c.setOnMouseReleased(this::handleMouseReleaseForSelection);
    }

    private void setupKeyboardShortcuts(Scene sc) {
        if (sc == null) return;
        sc.addEventFilter(KeyEvent.KEY_PRESSED, e -> {
            if (
                addressField.isFocused() && e.getCode() != KeyCode.ESCAPE
            ) return;
            switch (e.getCode()) {
                case DELETE:
                    deleteFiles();
    ...
    private void showWelcomeMessage() {
        Alert a = new Alert(Alert.AlertType.INFORMATION);
        a.setTitle("Welcome to Photo Manager Pro");
        a.setHeaderText("Modern Photo Management Application");
        a.setContentText(
        ...
        protected void updateItem(File item, boolean empty) {
            ...
            if (empty || item == null) {
                ...
                if (item.isDirectory()) {
                    ...
                    icon.setStyle(
                        "-fx-font-size:14px;-fx-text-fill:white;-fx-padding:0 2 0 0;"
                    );
                    setGraphic(icon);
                    setStyle(
                        "-fx-background-color:transparent;-fx-text-fill:white;-fx-font-size:13px;-fx-font-family:'Segoe UI',sans-serif;-fx-font-weight:500;"
                    );
                } else {
                    Label icon = new Label("\uD83D\uDCCE");
                    icon.setStyle(
